...
const MIGRATION_ID = "017_link_sections_across_all_terms"
...
export const name = "Link sections across all academic terms"
...
export async function up(context = {}) {
    ...
    if (!databaseId) {
        throw new Error("Missing databaseId for migration 017_link_sections_across_all_terms.")
    }
    ...
    for (const group of groupedSections.values()) {
        ...
        const shouldClearLegacyScope = legacyFields.length > 0 && LEGACY_SECTION_FIELDS.some((key) => str(canonical?.[key]))
    ...
    log.info?.(
        `✅ ${MIGRATION_ID}: merged ${mergedGroups} section group(s), removed ${removedDuplicates} duplicate section(s), rewired ${rewiredClasses} class reference(s), and cleared legacy term scope on ${clearedLegacyScope} canonical section(s).`
    )
}
