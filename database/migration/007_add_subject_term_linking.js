...
export const id = "007_add_subject_term_linking";
...
export async function up({ databases, databaseId }) {
    ...
    await waitForCollection(databases, databaseId, COLLECTIONS.ACADEMIC_TERMS);

    await ensureStringAttribute(
        databases,
        databaseId,
        COLLECTIONS.SUBJECTS,
        ATTR.SUBJECTS.termId,
        {
            size: 255,
            required: false,
            defaultValue: null,
    ...
    await ensureKeyIndex(
        ...
        databaseId,
        COLLECTIONS.SUBJECTS,
        INDEX.SUBJECTS.termId,
        [ATTR.SUBJECTS.termId]
    );

    console.log(
        `✅ Migration ${id} complete. Added optional ${COLLECTIONS.SUBJECTS}.${ATTR.SUBJECTS.termId} and index ${INDEX.SUBJECTS.termId}.`
    );
    console.log(
        "ℹ️ Existing subjects are not force-backfilled here. Unlinked subjects will be linked to the selected record term by the updated UI flow when saved."
    );
}
