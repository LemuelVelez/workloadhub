...
export const id = "009_add_subject_multi_scope_visibility";
...
export async function up({ databases, databaseId }) {
    ...
    console.log(
        `✅ Migration ${id} complete. Added optional subject scope fields (${ATTR.SUBJECTS.programId}, ${ATTR.SUBJECTS.programIds}, ${ATTR.SUBJECTS.yearLevel}, ${ATTR.SUBJECTS.yearLevels}, ${ATTR.SUBJECTS.semester}) and indexes for subject-to-program/year/semester filtering.`
    );
    console.log(
        "ℹ️ Existing subjects are not force-backfilled here. Update subjects through Master Data so they match the intended college, program, year level, and semester scope."
    );
}
